...
async def fetch_top_news():
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 402:
            ...
            return False, "Les actualités du jour n'ont pas pu être chargées (quota quotidien de l'API dépassé). Précise cela à l'utilisateur au début de ta réponse."
        if response.status_code == 429:
            print("Erreur World News API : trop de requêtes")
            return False, "Les actualités du jour n'ont pas pu être chargées (trop de requêtes envoyées à l'API). Précise cela à l'utilisateur au début de ta réponse."
        if response.status_code != 200:
            print(f"Erreur World News API : code {response.status_code}")
            return False, f"Les actualités du jour n'ont pas pu être chargées (erreur technique, code {response.status_code}). Précise cela à l'utilisateur au début de ta réponse."
...
async def get_daily_prompt():
    ...
    with Session(engine) as session:
        ...
        ok, content = await fetch_top_news()
        if ok:
